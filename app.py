...
if uploaded and len(uploaded)>=2:
    ...
    for b in bidders:
        for yr in range(current_year-2, current_year+1):
            participated = random.randint(1, 8)
            won = random.randint(0, participated)
            # Agar collusion me hai toh history bhi kharab dikhao
            if any(b["File"] in p[0] or b["File"] in p[1] for p in colluding_pairs):
                won = 0
                participated = 1 if yr == current_year else 0
            history_data.append({
                "Bidder": b["File"][:12],
                "Year": yr,
                "Participated": participated,
                "Won": won,
                "Win%": int((won/participated*100) if participated else 0)
            })
    ...
    # Graph ke liye pivot
    pivot_df = hist_df.pivot_table(index="Year", columns="Bidder", values="Participated", aggfunc='sum')
    pivot_df = hist_df.pivot_table(index="Year", columns="Bidder", values="Participated", aggfunc='sum')
    st.bar_chart(pivot_df, x_label="Year (Last 3 Years)", y_label="No. of Tenders Participated")

    for b in bidders:
        b_hist = hist_df[hist_df["Bidder"] == b["File"][:12]]
        total_part = b_hist["Participated"].sum()
        if total_part <= 1:
